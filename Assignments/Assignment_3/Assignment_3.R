...
head(letters,n=5)
#tells you the first 5 letters

tail(letters,n=5)
#tells you the lest 5 letters

letters[15]
#tells you the 15th letter
...
letters[c(1,5)] <- "asd"
letters
#changes the first and fifth letter to asd

letters[c(1,5)] <- c("a","e")
#changes the first and fifth letters back to a and e
...
class(vector3)
class(vector2)
#this tells you whether the vector is a character or numeric (could also be an integer but only whole numbers)

# Logical expressions (pay attention to these...they are used ALL THE TIME)
vector1 > 3 
#[1] FALSE FALSE FALSE  TRUE  TRUE  TRUE  TRUE  TRUE  TRUE  TRUE
#this answer refers to each number in the vector, false and numbers under 3 and the true are numbers above 3

vector1 >= 3
vector1 <= 5

class(vector1 < 5)
#the answer here is logical, which means that the numbers in vector 1 are under 5

vector1 == 7
#double equal signs means is equal to( asking a question)

letters == "a"
#answer is only true for the first letter a

letters != "c"
#exclamation mark means that that the question is not equal to, meaning everything is true except for letter C


letters %in% c("a","b","c","z")
# compares and matches says if letters have the listed letters

c("a","b") %in% letters
#this asks if the vector is present in the letters
...
names(dat)
dim(dat)
#tells you dimensions in rows and columns respectfully

head(dat)
#tells you the first 6 

tail(dat,n=1)
#tells you the very last one, counts from the back

dat$Species
class(dat$Species)
#factor is a categorical character that has to correspond to a level, identifies acceptable answers
...
str(dat)
#this tells you the structure of the data
...
dat[seq(2,150,2),]
#this gives you the numbers between 2 to 150 by 2, replacing rows with this and leaving columns blank
...
iris_chr <- iris
str(iris_chr)
as.character(iris_chr$Sepal.Length,iris_chr$Sepal.Width,iris_chr$Petal.Length,iris_chr$Petal.Width,iris_chr
             $Species)
#for this one, I must do each column individually, all together does not work

as.character(iris_chr$Sepal.Length)
as.character(iris_chr$Sepal.Width)
as.character(iris_chr$Petal.Length)
as.character(iris_chr$Petal.Width)

apply(iris,2,as.character)
